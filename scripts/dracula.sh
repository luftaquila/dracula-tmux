...
main()
{
  ...
  # Status left
  if $show_powerline; then
    tmux set-option -g status-left "#[bg=${yellow},fg=${dark_gray}]#{?client_prefix,#[bg=${pink}]#[fg=${white}],} ${left_icon} #[fg=${yellow},bg=${gray}]#{?client_prefix,#[fg=${pink}],}${left_sep}"
    powerbg=${gray}
  else
    tmux set-option -g status-left "#[bg=${yellow},fg=${dark_gray}]#{?client_prefix,#[bg=${pink}]#[fg=${white}],} ${left_icon}"
  fi
